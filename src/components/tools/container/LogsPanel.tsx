        ...
        {!atBottom && (
          <button
            ...
            className={cn(
              ...
              'border border-acc/40 bg-card/95 px-3 py-1 text-[11px] font-medium shadow-lg backdrop-blur-sm',
...
function IconStep({ title, onClick, children }: { title: string; onClick: () => void; children: ReactNode }) {
  return (
    <button
      ...
      aria-label={title}
      onClick={onClick}
      className="inline-flex h-5 w-5 items-center justify-center rounded-sm text-fg-mute transition-colors hover:bg-acc/10 hover:text-fg focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-acc/35"
    >
      {children}
    </button>
  );
