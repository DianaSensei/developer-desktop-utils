...
// Lớp lệnh giờ đến từ `useRabbitApi()` (dựng trên SDK của plugin), nên mock đặt
// ở đó thay vì ở `./types` — test không phải biết lớp lệnh được dựng bằng cách
// nào, chỉ cần biết bridge gọi những hàm nào.
vi.mock('./api_sdk', () => ({
  useRabbitApi: () => ({
