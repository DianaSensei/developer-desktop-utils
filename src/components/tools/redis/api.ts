...
import { createRedisApi, type RedisApi } from './types';

/**
 * Lớp lệnh Redis gắn với SDK của plugin.
 *
 ...
 * chạy được.
 */
export function useRedisApi(): RedisApi {
  const sdk = usePluginSdkFor('redis-client');
  return useMemo(() => createRedisApi(sdk), [sdk]);
}
