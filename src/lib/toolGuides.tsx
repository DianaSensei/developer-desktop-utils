  ...
  'api-client': (
    <>
      ...
      <GuideSection icon={FileCode2} title="Scripting reference">
        ...
        <p><strong className="text-fg">require(name)</strong> pulls in a small curated set of bundled libraries — never <Tok>require(anything)</Tok>, only these: <Tok>crypto-js</Tok>, <Tok>uuid</Tok>, <Tok>lodash</Tok>, <Tok>jwt-decode</Tok>, <Tok>dayjs</Tok>, <Tok>jose</Tok>, <Tok>jsonwebtoken</Tok>. <Tok>jose</Tok> and <Tok>jsonwebtoken</Tok> generate real JWS/JWT signatures — HMAC (HS256/384/512), RSA (RS256/384/512, PS256/384/512), ECDSA (ES256/384/512), and EdDSA — using the browser's Web Crypto API. <Tok>jsonwebtoken</Tok> here is a same-shaped stand-in for the real npm package (which needs Node's <Tok>crypto</Tok>/<Tok>Buffer</Tok> and can't run in this app at all): its <Tok>sign()</Tok>/<Tok>verify()</Tok> must be <Tok>await</Tok>ed, and <Tok>verify()</Tok> requires an explicit <Tok>algorithms</Tok> list rather than trusting the token's own header.</p>
