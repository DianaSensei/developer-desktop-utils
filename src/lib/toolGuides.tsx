...
export const TOOL_GUIDES: Record<string, ReactNode> = {
  ...
  'api-client': (
    <>
      ...
      <GuideSection icon={Braces} title="Variables">
        <p>Use <Var>{'{{name}}'}</Var> anywhere — URL, query, headers, body, or auth. A token turns <span className="text-ok">green</span> when it resolves in the current context and <span className="text-bad">red</span> when it doesn’t; hover to see its value. In the Name/Value tables (query, path, headers, url-encoded and form-data bodies) a <strong className="text-fg">Resolved</strong> column appears as soon as any row uses a token, showing what each one is worth right now — and naming, in red, any token nothing defines, which would otherwise be sent literally. Tables past a handful of rows also get their own filter box.</p>
        <p>Two environment pickers, top-right: a <strong className="text-fg">Collection</strong> one (scoped to one collection, follows whichever one the active request belongs to) and a <strong className="text-fg">Global</strong> one (applies everywhere, unaffected by which collection is active) — both can be active at once. Scripts write straight into a persisted store with <Tok>dt.setCollectionVar()</Tok> or <Tok>dt.setEnvVar(name, value, 'collection' | 'global')</Tok>.</p>
        <p>Variables come from four places — the active Collection environment, the active Global environment, Collection Variables, and the Vault — each with its own editor. When a name is defined in more than one: the Collection environment wins, then the Global environment, then Collection Variables; Vault entries are referenced as <Var>{'{{vault.name}}'}</Var> and never collide with the rest. The eye icon next to the environment pickers shows every one currently in scope, grouped by source, with a link straight to each editor.</p>
      </GuideSection>
      ...
      <GuideSection icon={ShieldCheck} title="Scripts, tests & cookies">
        <p>Each request has pre-request and post-response scripts plus tests/assertions (a Bruno-shaped <Tok>dt</Tok>/<Tok>req</Tok>/<Tok>res</Tok> API — same variable/flow-control surface as Bruno's own <Tok>bru</Tok>, under this app's own name instead — with a Postman <Tok>pm.*</Tok>/<Tok>postman.*</Tok> shim for imported Postman scripts). Collection/folder scripts are inherited.</p>
        <p>Scripts get <Tok>dt.setEnvVar</Tok>/<Tok>getEnvVar</Tok>/<Tok>setCollectionVar</Tok>/<Tok>getCollectionVar</Tok>, <Tok>dt.interpolate()</Tok> to expand <Var>{'{{tokens}}'}</Var>, and <Tok>dt.sleep(ms)</Tok>. <Tok>req</Tok> can rewrite the URL, method, headers, query params, and per-request timeout/redirect settings before the send. Assertions use a chai subset — <Tok>expect().to.deep.equal</Tok>, <Tok>.oneOf</Tok>, <Tok>.closeTo</Tok>, <Tok>.keys</Tok>, <Tok>.throw</Tok> — or the named <Tok>assert.*</Tok> helpers.</p>
      ...
      <GuideSection icon={FileCode2} title="Scripting reference">
        <p><strong className="text-fg">Execution order</strong> for one send: inherited pre-request scripts (collection, then folder, outer to inner) → the request's own pre-request script → the HTTP send → the request's own post-response script → inherited post-response scripts → the test script → declarative assertions. A pre-request failure stops the send entirely (a request built by a script that threw isn't worth sending); a post-response/test/assertion failure is recorded and everything after it still runs, so one broken test never hides the rest.</p>
        <p><strong className="text-fg">Globals available</strong>: <Tok>dt</Tok> (variables + flow control, above), <Tok>req</Tok> (pre-request only — mutate the outgoing request), <Tok>res</Tok> (post-response/test/assert only — read the response), <Tok>pm</Tok>/<Tok>postman</Tok> (Postman-compatibility shim over the same <Tok>dt</Tok>/<Tok>req</Tok>/<Tok>res</Tok> data), <Tok>test(name, fn)</Tok> + <Tok>expect(actual)</Tok> (chai-style BDD), <Tok>assert</Tok> (chai <Tok>assert</Tok> style), and <Tok>console.*</Tok> (captured as script logs, visible per-request).</p>
        <p><strong className="text-fg">require(name)</strong> pulls in a small curated set of bundled libraries — never <Tok>require(anything)</Tok>, only these: <Tok>crypto-js</Tok>, <Tok>uuid</Tok>, <Tok>nanoid</Tok>, <Tok>lodash</Tok>, <Tok>jwt-decode</Tok>, <Tok>dayjs</Tok>, <Tok>jose</Tok>, <Tok>jsonwebtoken</Tok>, <Tok>ajv</Tok>, <Tok>xml2js</Tok>, <Tok>cheerio</Tok> — the same set Postman's and Bruno's own script sandboxes bundle. <Tok>jose</Tok> and <Tok>jsonwebtoken</Tok> generate real JWS/JWT signatures — HMAC (HS256/384/512), RSA (RS256/384/512, PS256/384/512), ECDSA (ES256/384/512), and EdDSA — using the browser's Web Crypto API. <Tok>jsonwebtoken</Tok> here is a same-shaped stand-in for the real npm package (which needs Node's <Tok>crypto</Tok>/<Tok>Buffer</Tok> and can't run in this app at all): its <Tok>sign()</Tok>/<Tok>verify()</Tok> must be <Tok>await</Tok>ed, and <Tok>verify()</Tok> requires an explicit <Tok>algorithms</Tok> list rather than trusting the token's own header. <Tok>ajv</Tok> schema-validates a JSON response (<Tok>new (require('ajv'))().compile(schema)</Tok>), <Tok>xml2js</Tok> parses an XML/SOAP response into an object, and <Tok>cheerio</Tok> queries/scrapes an HTML response with a jQuery-like API.</p>
        <p><strong className="text-fg">Deliberately off-limits to scripts</strong>: the Vault (<Tok>store.vault</Tok>) never enters any script's scope, so a script — or one pasted in from an untrusted collection — can't read or exfiltrate a secret it was never given; secret-flagged environment variables get the same treatment. Declarative assertions are a restricted expression grammar (property paths, one method call, literals, arithmetic/comparison/logical operators) rather than real JS, so an imported collection's Assert rows can't run arbitrary code even before you've decided whether to trust its scripts.</p>
        <p><strong className="text-fg">MCP</strong>: an MCP client (Claude Code, Claude Desktop) can inspect and drive this exact tool — list/edit collections, requests, scripts, environments, and actually send a request through the same engine the Send button uses. By default it only answers while this tool is the one on screen; turn on <strong className="text-fg">Settings → MCP → Background MCP bridge</strong> to let it answer regardless of which tool you're looking at. Ask the MCP client to call <Tok>get_scripting_reference</Tok> for this same rundown in machine-readable form, including the exact <Tok>bru</Tok>→<Tok>dt</Tok> naming note above.</p>
      </GuideSection>
      <GuideSection icon={Play} title="Runner">
        ...
        <p><strong className="text-fg">Flow control</strong>: call <Tok>dt.setNextRequest("Login")</Tok> (or <Tok>pm.execution.setNextRequest</Tok> / <Tok>postman.setNextRequest</Tok>) from a script to jump to another request by name, or pass <Tok>null</Tok> to end the iteration early. Jumps are shown under the row that made them, so retry loops and conditional paths are visible in the results.</p>
        <p><strong className="text-fg">Require HTTP 2xx</strong> is on by default: every request gets a built-in check, so anything that isn’t 2xx — a redirect, a 404, no response at all — counts as failed and is reported like an assertion, without writing a test for it. Turn it off if redirects or error codes are expected outcomes for your collection.</p>
        <p>Other advanced options: <strong className="text-fg">stop the run on the first failure</strong>, and turn off <strong className="text-fg">save responses</strong> for long runs you don’t need to inspect afterwards (it defaults off above 2,000 data rows, so a very large run can’t exhaust memory — statistics still cover every row).</p>
        <p>Results show a live progress bar, wall-clock duration, average response time, total data received, an <strong className="text-fg">HTTP 2xx</strong> tally counted separately from assertions, and a per-status-code breakdown with min/avg/max timing — collected for every execution even when responses aren’t saved. Click a status code to jump to an iteration that returned it, or switch to <strong className="text-fg">By request</strong> to see every iteration rolled up per request: runs, HTTP 2xx, pass/fail and that request’s own response codes. Filter by passed/failed, or <strong className="text-fg">Export</strong> the run as CSV (one row per executed request, with the data-file columns that produced it) or as a JSON report: options used, summary counts, timings, per-request tests, console output, iteration data, and response bodies when kept. Either format can be exported <strong className="text-fg">failures only</strong>, which is usually what you want after a run over a large data file; both are written to disk in pieces, so even a very large run exports without loading the whole report into memory.</p>
      ...
      <GuideSection icon={FolderTree} title="Organize, import & export">
        <p>Group requests into collections and folders in the left sidebar; open several in tabs — each remembers the tab you were working in. Drag a request or folder to reorder it, drop it onto another folder or collection to move it there, or hold <Key>Alt</Key> while dragging to copy it instead — the drop target highlights green in copy mode, blue in move mode.</p>
        <p>Opening a request always expands its collection and folders if they were collapsed and scrolls it into view — a folder or collection you collapse while still working inside it gets a small dot instead, so you never lose track of where the open request lives.</p>
        <p><strong className="text-fg">History</strong> records every send you make by hand, grouped by day and searchable by URL, method or status, with an <strong className="text-fg">All / OK / Failed</strong> filter. Selecting an entry shows what was actually sent — general, query, path params, headers and body — and the captured response, as two switchable views rather than stacked. <Tok>↗</Tok> reopens any past send as a new request.</p>
  ...
  base64: makeGuide({
    use: [
      ...
      'Encrypt tab: AES-256 GCM by default — enter a passphrase and encrypt/decrypt. The AES-CBC/CTR/CFB/OFB, 3DES and Rabbit modes below it exist to exchange ciphertext with a crypto-js caller.',
    ...
    know: [
      ...
      'AES-256 GCM ciphertext is a self-describing envelope (magic, iteration count, salt, IV, tag), so raising the iteration count later still leaves older messages readable. It is this tool’s own format — crypto-js cannot read it.',
    ],
    caveat: [
      'Only AES-256 GCM stretches the passphrase properly (PBKDF2-SHA256, 600k rounds) and detects tampering. Every other mode is crypto-js, which derives its key with a single MD5 and authenticates nothing — interop, not protection. ECB additionally leaks plaintext patterns.',
  ...
  diff: makeGuide({
    use: [
      'Paste (or type into) the original on the left and the modified version on the right — both panes stay editable.',
      'Changes highlight inline as you type, with unchanged lines vertically aligned between the two sides so they stay in sync while scrolling.',
      <>Click the <Tok>⇝</Tok> arrow next to a changed chunk to revert it — copies that chunk from Original into Modified.</>,
