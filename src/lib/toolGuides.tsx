  ...
  'api-client': (
    <>
      ...
      <GuideSection icon={Braces} title="Variables">
        ...
        <p>Variables come from four places — the active Environment, Collection Variables, session Runtime Variables, and the Vault — each with its own editor. When a name is defined in more than one, the Environment wins, then Runtime, then Collection Variables; Vault entries are referenced as <Var>{'{{vault.name}}'}</Var> and never collide with the rest. The eye icon next to the environment picker shows every one currently in scope, grouped by source, with a link straight to each editor.</p>
      </GuideSection>
