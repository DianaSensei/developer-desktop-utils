...
- `require(...)` — a small curated set of bundled libraries, never `require(anything)`: `lodash`, `crypto-js`, `uuid`, `jwt-decode`, `dayjs`, `jose`, `jsonwebtoken`. `jose` and `jsonwebtoken` generate real JWS/JWT signatures — HMAC (HS256/384/512), RSA (RS256/384/512, PS256/384/512), ECDSA (ES256/384/512), and EdDSA — via the browser's Web Crypto API. `jsonwebtoken` here is a same-shaped stand-in for the real npm package, which needs Node's `crypto`/`Buffer` and cannot run in this app's webview at all: its `sign()`/`verify()` must be `await`ed, and `verify()` requires an explicit `algorithms` list rather than trusting the token's own header (closes the classic algorithm-confusion hole the real package leaves open by default).
