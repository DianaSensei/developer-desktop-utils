...
**Scripting (Bruno-shaped, own naming):** each request can have a pre-request script, a post-response script, a test script, and declarative assertions. Collections and folders can also carry pre/post scripts that are **inherited** by every request inside them (edit via the collection ⋮ menu → Scripts, or a folder's `</>` action) — pre-request runs collection → folder → request, post-response unwinds in reverse. Scripts are JavaScript with a curated API in scope:

- `dt` — get/set Collection Variables & environment variables (`getCollectionVar`/`setCollectionVar`, `getEnvVar`/`setEnvVar(name, value, 'collection' | 'global')`, `interpolate`, `sleep`, `setNextRequest`, …). Same shape as Bruno's own `bru` global, under this app's own name — a script pasted in from real Bruno that calls `bru.*` will not resolve `bru` and needs a find/replace to `dt` first; there is no compatibility alias for the name itself.
...
- `require(...)` — a small curated set of bundled libraries, never `require(anything)`: `lodash`, `crypto-js`, `uuid`, `nanoid`, `jwt-decode`, `dayjs`, `jose`, `jsonwebtoken`, `ajv`, `xml2js`, `cheerio` — the same set Postman's and Bruno's own script sandboxes bundle. `jose` and `jsonwebtoken` generate real JWS/JWT signatures — HMAC (HS256/384/512), RSA (RS256/384/512, PS256/384/512), ECDSA (ES256/384/512), and EdDSA — via the browser's Web Crypto API. `jsonwebtoken` here is a same-shaped stand-in for the real npm package, which needs Node's `crypto`/`Buffer` and cannot run in this app's webview at all: its `sign()`/`verify()` must be `await`ed, and `verify()` requires an explicit `algorithms` list rather than trusting the token's own header (closes the classic algorithm-confusion hole the real package leaves open by default). `ajv` (`new (require('ajv'))().compile(schema)`) schema-validates a JSON response; `xml2js` parses an XML/SOAP response into an object; `cheerio` (`require('cheerio').load(html)`) queries/scrapes an HTML response with a jQuery-like API; `nanoid` is a second, shorter/URL-safe id generator alongside `uuid`. Not bundled: `moment` (superseded by `dayjs`, already here), `chai` (this engine ships its own `expect`/`assert`, not a `require()`d module), and `tv4` (deprecated even in Postman's own docs, in favor of `ajv`).
- `pm` / `postman` — a Postman compatibility shim (`pm.environment`, `pm.collectionVariables`, `pm.globals`, `pm.response`, `pm.test`, `pm.expect`, `postman.setNextRequest`, …) so many imported Postman scripts run without rewriting — unlike `bru`, Postman's own names are kept as-is here for that reason.

**Execution order** for one send: inherited pre-request scripts (collection, then folder, outer to inner) → the request's own pre-request script → the HTTP send → the request's own post-response script → inherited post-response scripts → the test script → declarative assertions. A pre-request failure stops the send entirely (a request built by a script that threw is not worth sending); a post-response/test/assertion failure is recorded and everything after it still runs, so one broken test never hides the rest.

**Scripts execute in a sandboxed Web Worker**, off the main thread. The worker has the `dt`/`req`/`res`/`pm` API and the bundled `require` libraries, and **nothing else**: no DOM, no `localStorage`, and no Tauri IPC — so a script cannot reach the file system, the clipboard, or the shell. The Vault (`store.vault`) never enters a script's scope either, for the same reason — a script pasted in from an untrusted collection can't read or exfiltrate a secret it was never given; secret-flagged environment variables get the same treatment. Declarative assertions are a restricted expression grammar (property paths, one method call, literals, arithmetic/comparison/logical operators) rather than real JS, so an imported collection's Assert rows can't run arbitrary code either. A script that never returns is killed once it passes the **script timeout** (Settings → Configuration → API Client, default 5 s), so a runaway loop cannot freeze the app. Scripts never run on their own — only as part of a Send you initiate. `console.log` output and test results appear in the response panel. Every variable a script sets (`dt.setCollectionVar`/`setEnvVar`) persists — to Collection Variables or to the active environment — in `localStorage`.

**MCP:** an MCP client (Claude Code, Claude Desktop) can inspect and drive this exact tool through the bundled `devtool-mcp-server` sidecar — list/edit collections, requests, scripts, environments, and actually send a request through the same engine the Send button uses (see `docs/human/mcp-server.md` for setup). By default it only answers while this tool is the one on screen; Settings → MCP → **Background MCP bridge** (off by default) lets it answer regardless of which tool is on screen. Call its `get_scripting_reference` tool for this same rundown in machine-readable form.
...
**Postman compatibility:** import reads Postman Collection **v2.1** JSON (folders, requests, headers, query, body, bearer/basic auth, and pre-request/test scripts); export writes the same format. Postman scripts use the `pm.*` API — the script text is preserved on import so it keeps running via the `pm.*` shim, or you can adapt it to this app's own `dt`/`req`/`res` API. Import/export use the native file picker (desktop) or browser file input/download (web).
